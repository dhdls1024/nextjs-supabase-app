...
**최종 업데이트**: 2026-03-26
**진행 상황**: Phase 3 진행 중 (Task 012~017, 019 완료 / Task 018 대기 중)
...
- **Task 018: 결제일 PWA 푸시 알림 구현 (F008)** - 대기 중
  - PWA Web Push (FCM) 방식으로 구현 예정
  - DB: `notification_settings` 테이블 (알림 on/off, D-day 설정) + `push_subscriptions` 테이블 (FCM 토큰)
  - 프론트: Service Worker 등록, 푸시 구독 동의, FCM 토큰 저장
  - `components/notification-settings.tsx` Supabase 연동 (현재 UI만 구현됨)
  - Supabase Edge Function: `notify-upcoming-payments`
    - `subscriptions` 조회: `next_billing_date = today + N days` (사용자 설정값)
    - FCM API로 푸시 발송
  - pg_cron 설정: 매일 UTC 0시(한국 9시) Edge Function 호출 스케줄 등록

  ## 테스트 체크리스트
  - [ ] PWA 설치 후 알림 권한 허용 → FCM 토큰 DB 저장 확인
  - [ ] 결제일이 D-day인 구독 추가 후 Edge Function 수동 호출 → 푸시 알림 수신 확인
  - [ ] 알림 설정 토글 off 시 푸시 발송 중단 확인
...
- **Task 020: 성능 최적화 및 사용자 경험 개선**
  ...
  - Supabase 쿼리 최적화: 필요한 컬럼만 SELECT, 인덱스 확인
...
- **Task 022: Vercel 배포 파이프라인 구축**
  ...
  - Supabase Edge Function 프로덕션 배포 확인
  - `npm run build` 로컬 빌드 성공 확인 후 배포

  ## 테스트 체크리스트
  - [ ] Playwright MCP로 프로덕션 URL 접속 → 랜딩 페이지 정상 로드 확인
  - [ ] 프로덕션 환경에서 회원가입 → 로그인 → 구독 추가 플로우 확인
  - [ ] 프로덕션 환경에서 파일 업로드 및 다운로드 정상 동작 확인

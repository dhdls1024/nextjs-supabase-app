// Vercel Cron Route Handler: 결제 예정 구독 알림 발송
// vercel.json의 crons 설정으로 매일 00:00 UTC (09:00 KST)에 호출됨
// Supabase 무료 플랜 (pg_cron 미지원) 대신 Vercel Cron으로 스케줄 처리

import { NextResponse } from "next/server"
import webpush from "web-push"

// VAPID 인증 설정 — 환경변수에서 로드
...
export async function GET(request: Request) {
  // CRON_SECRET으로 Vercel Cron 요청 인증 — 무단 호출 방지
  ...
  // VAPID 키 등록 — webpush 라이브러리에 한 번만 설정
  webpush.setVapidDetails(VAPID_EMAIL, VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY)

  try {
    // service_role 권한으로 RLS 우회하여 전체 사용자 데이터 조회
    // 참고: createClient()는 일반 사용자 권한이므로 여기서는 서비스 롤 키 직접 사용
    ...
    if (!preferences || preferences.length === 0) {
      return NextResponse.json({ message: "알림 대상 사용자가 없습니다.", sent: 0 })
    }
    ...
    for (const pref of preferences) {
      // dday_offset만큼 앞선 날짜의 구독 조회
      // 예: dday_offset=1이면 내일 결제 구독에 대해 오늘 알림
      ...
      const subNames = subscriptions.map((s: { name: string }) => s.name).join(", ")
      ...
      for (const pushSub of pushSubs) {
        try {
          ...
          // 410 Gone 또는 404: 구독이 만료/해제됨 → DB에서 자동 삭제
    ...
    return NextResponse.json({ message: "알림 발송 완료", ...result })
  } catch (err) {
